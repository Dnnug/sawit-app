<x-app-layout>
    <x-slot name="header">
        <h2 class="font-semibold text-xl text-slate-800 leading-tight">Edit: {{ $blocks->name }}</h2>
    </x-slot>

    <div class="py-12">
        <div class="max-w-2xl mx-auto sm:px-6 lg:px-8">
            <form action="{{ route('blocks.update', $blocks->id) }}" method="POST"
                ...
                <div>
                    ...
                    <x-text-input id="name" name="name" type="text" class="mt-1 block w-full"
                        value="{{ old('name', $blocks->name) }}" required />
                </div>
                ...
                <div class="grid grid-cols-2 gap-4">
                    <div>
                        ...
                        <x-text-input id="area" name="area" type="number" step="0.1"
                            class="mt-1 block w-full" value="{{ old('area', $blocks->area) }}" required />
                    </div>
                    <div>
                        ...
                        <x-text-input id="year_planted" name="year_planted" type="number" class="mt-1 block w-full"
                            value="{{ old('year_planted', $blocks->year_planted) }}" required />
                    </div>
                </div>
                ...
                <div>
                    ...
                    <x-text-input id="tree_count" name="tree_count" type="number" class="mt-1 block w-full"
                        value="{{ old('tree_count', $blocks->tree_count) }}" />
                </div>

                <div>
                    <x-input-label for="description" value="Keterangan" />
                    <textarea id="description" name="description"
                        class="mt-1 block w-full border-slate-300 focus:border-green-500 focus:ring-green-500 rounded-lg shadow-sm">{{ old('description', $blocks->description) }}</textarea>
                </div>
